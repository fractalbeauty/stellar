        ...
        loop {
            tokio::select! {
                ...
                Some((side, connection)) = connection_rx.recv() => {
                    ...
                    let peer = PeerTask::spawn(side, cancellation_token.child_token(), peer_exited_tx.clone(), connection_id, connection);
    ...
    pub fn spawn(
        ...
        tokio::spawn({
            ...
            async move {
                ...
                let result = peer.run(side, cancellation_token, connection).await;
...
const ALPN: &'static [u8] = b"stellar-sync/1";
